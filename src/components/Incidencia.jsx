...
export default function Incidencias() {
  ...
  const confirmarIncidencia = async () => {
    try {
      const incidenciaData = await API.graphql(
        graphqlOperation(updateIncidencia, { input: { id: id, estado: true } })
      );
  ...
  const rechazarIncidencia = async () => {
    try {
      const incidenciaData = await API.graphql(
        graphqlOperation(updateIncidencia, { input: { id: id, estado: false } })
      );
      const incidenciaDetails = incidenciaData.data.updateIncidencia;
    ...
    <div class="p-10">
      <div class="p-8 bg-white shadow mt-10">
        <div class="grid grid-cols-1 md:grid-cols-3">
          ...
          <div class="space-x-8 flex justify-between mt-30 md:mt-0 md:justify-center"></div>
        </div>

        <div class="mt-10 text-left border-b pb-12">
          <h1 class="text-4xl font-medium text-gray-700">
            ID: <span class="font-light text-gray-500">{incidencia.id}</span>
          </h1>
          ...
          <div class="mt-10 text-right border-b pb-10">
            ...
            <button
              onClick={() => {
                confirmarIncidencia(incidencia.id);
              }}
            ...
            <button
              onClick={() => {
                rechazarIncidencia(incidencia.id);
              }}
